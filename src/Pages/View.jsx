...
const View = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get(`${API_URL}/product/public`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-100 via-indigo-100 to-purple-100 flex flex-col px-4 sm:px-6 py-10">
      ...
      {item.length === 0 ? (
        ...
        >
          <div className="bg-white/60 backdrop-blur-md rounded-2xl shadow-lg p-8 sm:p-10 max-w-md">
            ...
            <Link to="/addCard">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="px-6 py-3 bg-yellow-400 text-black rounded-xl font-semibold hover:bg-yellow-300 transition shadow-md hover:shadow-lg"
              >
                Add New Item
              </motion.button>
            </Link>
